  ...
  physics: {
    ...
    articles: [
      ...
      },
      {
        ...
        description:
          "Breaking down Einstein's famous theory into digestible concepts...",
        readTime: "15 min",
        category: "Physics",
        icon: <Atom size={20} color="#FBBF24" />,
    ...
    articles: [
      {
        ...
        description:
          "Exploring the patterns and mysteries behind prime numbers...",
        readTime: "8 min",
      ...
      },
      {
        ...
        description:
          "A visual approach to understanding the fundamentals of calculus...",
        readTime: "12 min",
        category: "Mathematics",
        icon: <Binary size={20} color="#FBBF24" />,
    ...
    articles: [
      {
        ...
        description:
          "Exploring the latest features in JavaScript and how to use them...",
        readTime: "10 min",
      ...
      },
      {
        ...
        description:
          "Getting started with Python in the field of data science...",
        readTime: "15 min",
        category: "Programming",
        icon: <Code size={20} color="#FBBF24" />,
        author: "Lisa Zhang",
...
const BOOKMARKS_STORAGE_KEY = "@article_bookmarks";

export default function Articles() {
  const router = useRouter();
  const [articles, setArticles] = useState<TopicArticles>(allArticles);
  ...
  const loadBookmarks = async () => {
    try {
      ...
      if (storedBookmarks) {
        const bookmarkedIds = new Set(JSON.parse(storedBookmarks));

        const updatedArticles = { ...allArticles };
        Object.keys(updatedArticles).forEach((topic) => {
          updatedArticles[topic].articles = updatedArticles[topic].articles.map(
            (article) => ({
              ...article,
  ...
  const toggleBookmark = async (articleId: string) => {
    try {
      ...
      let bookmarkedArticleFound = false;

      Object.keys(updatedArticles).forEach((topic) => {
        updatedArticles[topic].articles = updatedArticles[topic].articles.map(
          (article) => {
            if (article.id === articleId) {
              bookmarkedArticleFound = true;
      ...
      if (!bookmarkedArticleFound) return;

      setArticles(updatedArticles);

      const bookmarkedIds = Object.values(updatedArticles)
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {Object.entries(articles).map(([key, topicData]) => (
          <View key={key} style={styles.section}>
            <View style={styles.topicHeader}>
              <View style={styles.topicIcon}>{topicData.icon}</View>
                ...
                >
                  <View style={styles.articleHeader}>
                    <View style={styles.categoryContainer}>
                      <View style={styles.categoryIcon}>{article.icon}</View>
